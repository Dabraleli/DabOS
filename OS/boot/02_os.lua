...
function os.startAsShell(file)
	local result, reason = os.execute(file)
    if not result then
...
os.setenv("AUTORUNSET", "false")
os.setenv("AUTORUN", "")
os.setenv("AUTORUNTIMEOUT", 5)

if fs.exists("/home/autorun.cfg") then
  autorunconfig = io.open("/home/autorun.cfg", "r")
  io.input(autorunconfig)
  data = require("serialization").unserialize(io.read("*a"))
  os.setenv("AUTORUNSET", data["enabled"])
  os.setenv("AUTORUN", data["file"])
  os.setenv("AUTORUNTIMEOUT", tonumber(data["timeout"]))
  io.input(io.stdin)
end
