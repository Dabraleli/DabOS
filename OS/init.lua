do
  _G._OSVERSION = "Dabraleli OS 0.3"
  ...
  if not filesystem.isDirectory("/home") then
  ...
  if require("filesystem").exists("/home/screen.cfg") then 
    file = io.open("/home/screen.cfg")
    io.input(file)
    data = require("serialization").unserialize(io.read("*a"))
    component.gpu.setResolution(data["width"], data["height"])  
    io.input(io.stdin)
  end  
  status("Initializing system...")
  os.execute("rm -rf /syslog.log")
  require("term").clear()
...
while true do
  print(_OSVERSION .. " with " .. math.floor(require("computer").totalMemory() /1024) .. "k RAM, rootfs on " .. require("computer").getBootAddress():sub(1,3))
  if os.getenv("AUTORUNSET") == "true" then 
    print("Autorun entry found. Press any key to start shell.")
    result = nil
    for i = 1, tonumber(os.getenv("AUTORUNTIMEOUT")) do
    	if i ~= os.getenv("AUTORUNTIMEOUT") + 1 then print(os.getenv("AUTORUNTIMEOUT") + 1 - i .. " seconds left.")
    	else print(os.getenv("AUTORUNTIMEOUT") + 1 - i .. " second left.") end
    	result = require("event").pull(1, "key")
    	if result ~= nil then break end
    end
    if result == nil then
      os.startAsShell(os.getenv("AUTORUN"))
    else
      os.startAsShell(os.getenv("SHELL"))
    end
  else 
    os.startAsShell(os.getenv("SHELL"))
  end
